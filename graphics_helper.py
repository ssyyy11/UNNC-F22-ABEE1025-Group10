# Cite: CMU 15-112-f21 https://www.cs.cmu.edu/~112/notes/notes-graphics.html

# Require Python 3.6 or later
import sys
if ((sys.version_info[0] != 3) or (sys.version_info[1] < 6)):
    raise Exception('graphics_helper.py requires Python version 3.6 or later.')
...
class App(object):
    ...
    def _printUserTraceback(app, exception, tb):
        ...
        printLines = [ ]
        for line in lines:
            if (('"graphics_helper.py"' not in line) and
                ('/graphics_helper.py' not in line) and
                ('\\graphics_helper.py' not in line) and
...
print(f'Loaded graphics_helper version {App.version} (last updated {App.lastUpdated})')

if (__name__ == '__main__'):
    try: import graphics_helper_tests
    except: pass
